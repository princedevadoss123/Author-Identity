...
const FileHandler = require('../services/file-operations');
let Log = require('log');
let logger = new Log();
...
function _addCertificates(fileHandler) {
    return {
        key: fileHandler.readFile('server-module/https-keys/server.key'),
        cert:  fileHandler.readFile('server-module/https-keys/server.crt')
    }
}
